...
/***/ (function(module, exports) {
	...
	var config = {
	  apiOrigins: {
	    production: 'https://mysterious-plains-14810.herokuapp.com',
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var mapPage = __webpack_require__(31);
	var getFormFields = __webpack_require__(32);
	var mapEvents = __webpack_require__(33);
	...
	var onSignUp = function onSignUp(event) {
	  var data = getFormFields(this); // this will refer to event.target because it gets passed into addHandlers as a callback.
	  ...
	  .then(ui.signUpSuccess).then(function () {
	...
	var onSignIn = function onSignIn(event) {
	  // stop here , add console to check if code is working so far
	  event.preventDefault();
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var formatDate = __webpack_require__(37);

	// templates
	var editFormTemplate = __webpack_require__(43);
	var showStateItemCreateTemplate = __webpack_require__(44);
	var showStateAllTemplate = __webpack_require__(45);
	var stateDefaultItem = __webpack_require__(41);

	var deleteItem = function deleteItem(event) {
	  var id = $(event.target).attr('data-id');
	  var region = $(event.target).attr('data-state');
	  onDestroyItem(id, region);
	};

	var showSelectedItem = function showSelectedItem(event) {
	  // on click of the display button ('#display-button'),
	  // display the state-default items handlebar
	  var id = $(event.target).attr('data-id');
	  api.showItem(id).then(ui.getItemSuccess).catch(ui.getItemsFailure);
	  $('#create-button').show();
	};

	var goBacktoMap = function goBacktoMap() {
	...
	var onGetItems = function onGetItems(element, code, region) {
	...
	var myGoals = function myGoals() {
	  api.getItems().then(function (data) {
	    ui.getmyGoalsSuccess(data);
	  }).then(usMap).catch(ui.getmyGoalsFailure);
	};

	var onUpdateItem = function onUpdateItem(event) {
	  var content = getFormFields(event.target);
	  event.preventDefault();
	  api.updateItem(content).then(ui.updateItemSuccess).catch(ui.updateItemFailure);
	};

	var onDestroyItem = function onDestroyItem(id, region) {
	  // const content = getFormFields(event.target)
	  event.preventDefault();
	  api.destroyItem(id).then(ui.destroyItemSuccess).then(function () {
	...
	var onCreateItem = function onCreateItem(event) {
	  ...
	  api.createItem(newData).then(ui.createItemSuccess).then(function () {
	...
	var onSaveEdit = function onSaveEdit(event) {
	  ...
	  event.preventDefault();
	  api.saveEdit(newContent, id).then(function (data) {
	...
	var showEditForm = function showEditForm(event) {
	  var attributes = ['data-title', 'data-category', 'data-state', 'data-status', 'data-description', 'data-location', 'data-id', 'data-date'];
	  var placeHolders = ui.getAttribute(event.target, attributes);
	  var date = new Date(placeHolders['data-date']);
	  var month = date.getUTCMonth() + 1;
	  var day = date.getUTCDate();
	  var year = date.getUTCFullYear();
	  var forDisplay = month + '/' + day + '/' + year;
	  placeHolders['data-date'] = forDisplay;
	  var editFormHtml = editFormTemplate({ item: placeHolders });
	  $('#create-item-container').html(editFormHtml);
	  editHandlers();
	  $('#cancel-update').on('click', cancelUpdate);
	};

	var showStateView = function showStateView(items) {
	  store.currentItems = items;
	  var sortedData = items.items.sort(function (a, b) {
	    a = new Date(a.due_date);
	    b = new Date(b.due_date);
	    return a > b ? 1 : a < b ? -1 : 0;
	  });
	  var sortedDataObject = { items: sortedData };
	  var itemByState = showStateAllTemplate(sortedDataObject);
	  $('#state-view').html(itemByState);
	  createFormHandler();
	  var incompleteItems = sortedData.filter(function (item) {
	    return item.status === 'incomplete';
	  });
	  var completeItems = sortedData.filter(function (item) {
	    return item.status !== 'incomplete';
	  });

	  if (incompleteItems.length > 0) {
	    var nextIncompleteItem = incompleteItems[0];
	    ...
	    var nextCompleteItem = completeItems[completeItems.length - 1];
	...
	var usMap = function usMap() {
	  $('#vmap').vectorMap({
	    ...
	    selectedRegions: null,
	    showTooltip: false,
	    onRegionOver: function onRegionOver(element, code, region) {
	...
	var createFormHandler = function createFormHandler() {
	...
	var editHandlers = function editHandlers() {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var createItem = function createItem(content) {
	  return $.ajax({
	    method: 'POST',
	    url: config.apiOrigin + '/items',
	    headers: {
	...
	var updateItem = function updateItem(content) {
	  return $.ajax({
	    method: 'PATCH',
	...
	var destroyItem = function destroyItem(id) {
	  return $.ajax({
	    method: 'DELETE',
	    url: config.apiOrigin + '/items/' + id,
	    headers: {
	...
	var saveEdit = function saveEdit(newContent, id) {
	  return $.ajax({
	    method: 'PATCH',
	    url: config.apiOrigin + '/items/' + id,
	    headers: {
	...
	var updateAfterEdit = function updateAfterEdit(id) {
	  return $.ajax({
	    method: 'GET',
	    url: config.apiOrigin + '/items/' + id,
	    headers: {
...
/***/ (function(module, exports, __webpack_require__) {
	...
	// templates
	var addItemToList = __webpack_require__(38);
	var allGoals = __webpack_require__(39);
	var nextGoal = __webpack_require__(40);
	var stateDefaultItem = __webpack_require__(41);
	var backToMapTemplate = __webpack_require__(42);

	var updateAfterEditSuccess = function updateAfterEditSuccess(data) {
	  var $list = $('#item-list').children('.checkbox-item').children('button');
	  var $buttonToUpdate = $list.filter(function (x, e, a) {
	    var current = $(e).attr('data-id');

	    return current === data.item._id;
	  });
	  var addItemToListHtml = addItemToList({ item: data.item });
	  var $checkBoxToUpdate = $buttonToUpdate.parents('.checkbox-item');
	  $($checkBoxToUpdate).replaceWith(addItemToListHtml);
	};

	var updateAfterEditFailure = function updateAfterEditFailure(error) {};
	var saveEditFailure = function saveEditFailure(error) {};

	var hideMap = function hideMap() {
	  $('#map-view-container').hide();
	};

	var getAttribute = function getAttribute($button, array) {
	  var placeHolders = {};
	  array.forEach(function (e) {
	    var val = $($button).attr(e);
	    placeHolders[e] = val;
	  });
	  return placeHolders;
	};

	var getItemsSuccess = function getItemsSuccess(data, region) {
	  store.state = region;
	...
	var getItemsFailure = function getItemsFailure(data) {};

	var getmyGoalsSuccess = function getmyGoalsSuccess(data) {
	...
	var getmyGoalsFailure = function getmyGoalsFailure(data) {};

	var createItemSuccess = function createItemSuccess(data) {
	...
	var createItemFailure = function createItemFailure(data) {};

	var getItemSuccess = function getItemSuccess(data) {
	...
	var getItemFailure = function getItemFailure(data) {};

	var updateItemSuccess = function updateItemSuccess(data) {};

	var updateItemFailure = function updateItemFailure(data) {};

	var destroyItemSuccess = function destroyItemSuccess(data) {};

	var destroyItemFailure = function destroyItemFailure(data) {};
	...
	module.exports = {
	  ...
	  getItemFailure: getItemFailure,
	  updateItemSuccess: updateItemSuccess,
	  updateItemFailure: updateItemFailure,
	  destroyItemFailure: destroyItemFailure,
	  destroyItemSuccess: destroyItemSuccess,
	  getAttribute: getAttribute,
	  getmyGoalsSuccess: getmyGoalsSuccess,
	  getmyGoalsFailure: getmyGoalsFailure,
	  saveEditFailure: saveEditFailure,
	  updateAfterEditFailure: updateAfterEditFailure,
	  updateAfterEditSuccess: updateAfterEditSuccess
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	  ...
	  return "<div class=\"checkbox-item\">\n\n      <span>"
	    ...
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.title : stack1), depth0))
	    + "\"\n        data-category=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.category : stack1), depth0))
	    + "\"\n        data-status=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.status : stack1), depth0))
	    + "\"\n        data-description=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.description : stack1), depth0))
	    + "\"\n        data-date=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.due_date : stack1), depth0))
	    + "\"\n        data-state=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.state : stack1), depth0))
	    + "\"\n        data-id=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\"\n        data-location=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.location : stack1), depth0))
	    + "\"\n        class=\"btn-btn-default inline display-details-button\"\n        type=\"button\">\n          Details\n      </button>\n      <button\n        data-title=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.title : stack1), depth0))
	    + "\"\n        data-category=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.category : stack1), depth0))
	    + "\"\n        data-status=\""
	    ...
	    + "\"\n        data-location=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.location : stack1), depth0))
	    + "\"\n        class=\"btn-btn-default inline edit-button\"\n        type=\"button\">\n          Edit\n      </button>\n      <button\n        data-title=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.title : stack1), depth0))
	    + "\"\n        data-category=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.category : stack1), depth0))
	    + "\"\n        data-status=\""
	    ...
	    + "\"\n        data-location=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.location : stack1), depth0))
	    + "\"\n        class=\"btn-btn-default inline delete-button\"\n        type=\"button\">\n          Delete\n      </button>\n</div>\n";
	},"useData":true});
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	  ...
	  return "<div id=\"goal-response-container\" class=\"goal-response-container\" data-id=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.id : stack1), depth0))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	  ...
	  return "<div id=\"state-default-item\" class=\"display-item\" data-id=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\">\n  <h3 class='detail-header'>Your Next Item:</h3>\n  <h3>Title: "
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.title : stack1), depth0))
	    + "</h3>\n  <h4>Due Date: "
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.due_date : stack1), depth0))
	    + "</h4>\n  <h4>Location: "
	    ...
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.description : stack1), depth0))
	    + "</h4>\n  <h4>Date added: "
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.createdAt : stack1), depth0))
	    + "</h4>\n  <!-- <h4>"
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.isPublic : stack1), depth0))
	    + "</h4> -->\n  <h4>Category: "
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.category : stack1), depth0))
	    + "</h4>\n  <h4>Status: "
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.status : stack1), depth0))
	    + "</h4>\n  <!-- When click 'edit' button, goes to state-item-update.handlebars view -->\n  <!-- <button data-id=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" class=\"glyphicon glyphicon-edit\" type=\"button\" name=\"Edit\"></button> -->\n  <!-- When click 'delete' button, deletes item -->\n  <!-- <button data-id=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" class=\"glyphicon glyphicon-remove\" type=\"button\" name=\"Delete\"></button> -->\n</div>\n";
	},"useData":true});
...
/***/ (function(module, exports, __webpack_require__) {
	...
	function __default(obj) { return obj && (obj.__esModule ? obj["default"] : obj); }
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	    return "<button id=\"back-to-map-button\" class=\"back-to-map-button\" type=\"button\" name=\"button\"><span class=\"glyphicon glyphicon-menu-left\"></span>Back to map</button>\n";
	},"useData":true});
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	  ...
	  return "<!-- Empty form to create new item -->\n<h3>Edit this goal!</h3>\n<form id=\"edit-item\"\n  class=\"edit-item\"\n  data-title=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1["data-title"] : stack1), depth0))
	    + "\"\n  data-category=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1["data-category"] : stack1), depth0))
	    + "\"\n  data-status=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1["data-status"] : stack1), depth0))
	    + "\"\n  data-description=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1["data-description"] : stack1), depth0))
	    + "\"\n  data-date=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1["data-date"] : stack1), depth0))
	    + "\"\n  data-state=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1["data-state"] : stack1), depth0))
	    + "\"\n  data-id=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1["data-id"] : stack1), depth0))
	    + "\"\n  data-location=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1["data-location"] : stack1), depth0))
	    + "\">\n\n  <fieldset>\n    <p1>Title:<input class=\"form-control\" type=\"text\" name=\"items[title]\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1["data-title"] : stack1), depth0))
	    + "\" required></p1>\n    <p1>Due Date:<input class=\"form-control\" type=\"text\" onfocus=\"(this.type='date')\" onblur=\"(this.type='text')\"  name=\"items[due_date]\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1["data-date"] : stack1), depth0))
	    + "\" required></p1>\n    <p1>Location:<input class=\"form-control\" type=\"text\" name=\"items[location]\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1["data-location"] : stack1), depth0))
	    + "\" required>\n    <p1>Description:<input class=\"form-control\" type=\"text\" name=\"items[description]\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1["data-description"] : stack1), depth0))
	    + "\" required>\n    <p1>Category:<input class=\"form-control\" type=\"text\" name=\"items[category]\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1["data-category"] : stack1), depth0))
	    + "\" required>\n    <p1>Status:<input class=\"form-control\" type=\"text\" name=\"items[status]\" value=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1["data-status"] : stack1), depth0))
	    + "\" required>\n    <button data-id=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" class=\"glyphicon glyphicon-ok-circle\" type=\"submit\" name=\"Submit\"></button>\n    <button data-id=\""
	    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" id=\"cancel-update\" class=\"glyphicon glyphicon-remove-circle cancel-update\" type=\"button\" name=\"Cancel\"></button>\n  </fieldset>\n</form>\n";
	},"useData":true});
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
	  ...
	  return "<!-- Empty form to create new item -->\n<h3>Add to your bucket list for this state!</h3>\n<form id=\"create-item\" class=\"create-item\" data-id=\""
	    + alias1(((helper = (helper = helpers.item || (depth0 != null ? depth0.item : depth0)) != null ? helper : helpers.helperMissing),(typeof helper === "function" ? helper.call(depth0 != null ? depth0 : (container.nullContext || {}),{"name":"item","hash":{},"data":data}) : helper)))
	    + "\">\n    <input class=\"form-control\" type=\"text\" name=\"item[title]\" placeholder=\"Goal\" required>\n    <input class=\"form-control\" type=\"text\" onfocus=\"(this.type='date')\" onblur=\"(this.type='text')\"  name=\"item[due_date]\" placeholder=\"Due date\" required>\n    <!-- <input type=\"text\" name=\"item[state]\" placeholder=\"State\"> -->\n    <input class=\"form-control\" type=\"text\" name=\"item[location]\" placeholder=\"Location\" required>\n    <input class=\"form-control\" type=\"text\" name=\"item[description]\" placeholder=\"Description\" required>\n    <input class=\"form-control\" type=\"text\" name=\"item[category]\" placeholder=\"Category\" required>\n    <!-- <input class=\"form-control\" type=\"text\" name=\"item[status]\" placeholder=\"Status\"> -->\n    <!-- <button data-id=\""
	    + alias1(alias2(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" class=\"glyphicon glyphicon-ok-circle\" type=\"button\" name=\"Add\">Add</button> -->\n    <input class=\"form-control\" class=\"glyphicon glyphicon-ok-circle\" data-id=\""
	    + alias1(alias2(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.id : stack1), depth0))
	    + "\" type=\"submit\" name=\"Submit\" value=\"Create\">\n\n    <button id=\"cancel-create\" class=\"cancel-create\" data-id=\""
	    + alias1(alias2(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.id : stack1), depth0))
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
	  ...
	  return "      <div class=\"checkbox-item\">\n            <span>"
	    ...
	    + "\"\n              data-date=\""
	    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.due_date : stack1), depth0))
	    + "\"\n              data-state=\""
	  ...
	  return "<h2 id=\"state-header\" class=\"state-header\"></h2>\n      <h3 class=\"userMessage\" style=\"text-align:center;\"></h3>\n<!-- req.body.item[i].title -->\n<div class=\"row checkbox\">\n  <div class=\"col-sm-8\" id=\"item-list\">\n"
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var signUp = function signUp(data) {
	  ...
	  return $.ajax({
	    ...
	    data: data // this line is same as data: data. Need a new key on this object, just calling it data
	  });
	}; // an object we'll pass into as an ajax request

	var signIn = function signIn(data) {
	  // data object passed into  argument will be different from argument in above function
	  return $.ajax({
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var signUpSuccess = function signUpSuccess(response) {
	  // argument can be (response) or something too. Just an argument name
...
/***/ (function(module, exports, __webpack_require__) {
	...
	module.exports = __webpack_require__.p + "c93f930d42f9698fc0e2ecbf30cb010e.jpg";
